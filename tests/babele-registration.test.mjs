...
for (const [language, expected] of [
    ...
    test(`registration waits for core settings, then uses configured language ${language}`, () => {
        ...
        hooks.get("babele.init")({ register(config) { assert.equal(config.module, "translate-dnd5e-tashas-cauldron"); assert.equal(config.dir, "compendium"); calls.push(config.lang); } });
...
for (const language of ["es", "es-ES", "en", "en-US", "fr"]) {
    test(`converters are inactive outside Spanish: ${language}`, async () => {
        ...
        try {
            ...
            ready = true;
            hooks.get("setup")();
            assert.equal(typeof converters?.tcoeActivitiesById, language.startsWith("es") ? "function" : "undefined");
            if (converters) {
                for (const pack of ["actors", "character-options", "content", "magic-items", "dm-tools", "scenes", "tables"]) {
                    const data = JSON.parse(readFileSync(new URL(`../compendium/dnd-tashas-cauldron.tcoe-${pack}.json`, import.meta.url)));
